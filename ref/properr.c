	...
	for(i = 0; i < network->outputs; i++) {
		...
		for(j = 0; j < network->npl + 1; j++) {
			if(network->neurons[level][j] > 0) {
				...
				delta = NN_DELTA * error[i];
				network->deltas[level][j][i] += delta;
				deltas[j] += delta;
			}
		}
		level--;
			...
			for(j = 0; j < network->npl + 1; j++) {
				...
				if(network->neurons[level][j] > 0) {
					for(k = 0; k < network->npl; k++) {
						delta += NN_DELTA * deltasC[j] * weightsC[k];
						network->deltas[level][j][k] += NN_DELTA * weightsC[k];
		...
		for(j = 0; j < network->inputs + 1; j++) {
			for(k = 0; k < network->npl; k++) {
				network->deltas[0][j][k] += network->deltas[j] * weights[k];
			}
		}
	}
}
