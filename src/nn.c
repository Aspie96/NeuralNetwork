...
static inline void nn_cloneFunc(int count1, int count2, double **matrixout, double **matrixin) {
	int i;
	for(i = 0; i < count1; i++) {
		memcpy(matrixout[i], matrixin[i], sizeof(double) * count2);
	}
}
...
NN *nn_clone(const NN *network) {
	...
	retVal = nn_create(network->inputs, network->layers, network->npl, network->outputs, 0);
	nn_cloneFunc(network->inputs + 1, network->npl, retVal->weights[0], network->weights[0]);
	for(i = 1; i < network->layers; i++) {
		nn_cloneFunc(network->npl + 1, network->npl, retVal->weights[i], network->weights[i]);
	}
	nn_cloneFunc(network->npl + 1, network->outputs, retVal->weights[network->layers], network->weights[network->layers]);

	return retVal;
}


static inline double *nn_getWeights(int size, int initialize) {
	double *retVal;
	int i;
...
NN *nn_create(int inputs, int layers, int npl, int outputs, int initialize) {
	...
	for(j = 0; j < inputs + 1; j++) {
		retVal->weights[0][j] = nn_getWeights(npl, initialize);
#ifdef NN_MOMENTUM
...
#endif /* NN_MOMENTUM */
		for(j = 0; j < npl + 1; j++) {
			retVal->weights[i][j] = nn_getWeights(npl, initialize);
#ifdef NN_MOMENTUM
...
#endif /* NN_MOMENTUM */
	for(j = 0; j < npl + 1; j++) {
		retVal->weights[layers][j] = nn_getWeights(outputs, initialize);
#ifdef NN_MOMENTUM
...
#ifdef NN_MOMENTUM
			free(network->momentum[i][j]);
#endif /* NN_MOMENTUM */
		}
		free(network->weights[i]);
#ifdef NN_MOMENTUM
		free(network->momentum[i]);
#endif /* NN_MOMENTUM */
	}
	for(j = 0; j < network->inputs + 1; j++) {
		free(network->weights[0][j]);
#ifdef NN_MOMENTUM
		free(network->momentum[0][j]);
#endif /* NN_MOMENTUM */
	}
	free(network->weights[0]);
	free(network->weights);
#ifdef NN_MOMENTUM
	free(network->momentum[0]);
	free(network->momentum);
#endif /* NN_MOMENTUM */
...
static inline void nn_forwardPropagateFunc(NN *network, int *layer, int inc, int outc) {
	int i, j;
	network->neurons[*layer - 1][inc - 1] = 1;
	for(i = 0; i < outc; i++) {
...
NN *nn_unserialize(FILE *fp) {
	...
	retVal = nn_create(inputs, layers, npl, outputs, 0);
